...
window.addEventListener("scroll", ()=>{
  ...
  laptops[1].style.transform = `translateX(calc(630vh - ${offsetY}px))`
  laptops[2].style.transform = `translateX(calc(740vh - ${offsetY}px))`
  laptops[3].style.transform = `translateX(calc(810vh - ${offsetY}px))`
  // phones[1].style.transform = `translateX(calc(750vh - ${offsetY}px))`
})

var typed = new Typed('.typed-text', {
  strings: ['Rafi', 'a Biomedical Engineer', 'a Font End Developer', 'interested in Full-stack Development'],
  typeSpeed: 200,
  loop: true
})